...
type SortField = "name" | "parent" | "products"
...
export default function ProductCategoriesPage() {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/admin/products/categories?t=${timestamp}`, {
        cache: 'no-store'
      })
      
      if (!response.ok) {
        throw new Error('Erreur lors du chargement des catégories de produits')
      }
      
      const data = await response.json()
      console.log('Catégories de produits récupérées:', data)
      ...
      setError('Une erreur est survenue lors du chargement des catégories de produits')
    } finally {
      setIsLoading(false)
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/products/categories/${selectedCategoryId}`, {
        method: 'DELETE',
      })
      ...
      toast({
        title: 'Succès',
        description: 'Catégorie de produit supprimée avec succès'
      })
    } catch (error) {
      ...
      toast({
        variant: 'destructive',
        title: 'Erreur',
        description: 'Erreur lors de la suppression de la catégorie de produit'
      })
    } finally {
  ...
  return (
    <>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Supprimer cette catégorie de produit ?</AlertDialogTitle>
            <AlertDialogDescription>
              Cette action est irréversible. La catégorie sera définitivement supprimée.
              Tous les produits associés perdront leur catégorie.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
