...
export default function CyberCafePage() {
  ...
  const loadTickets = async () => {
    try {
      ...
      const todayUsage: {[key: string]: {used: number, broken: number, brokenDetails: BrokenTicket[]}} = {};
      ...
      // Mettre à jour les tickets avec les données d'usage
      ...
      // Calculer le revenu total du jour
      const totalRevenue = data.reduce((sum: number, ticket: TicketType) => {
        const usage = todayUsage[ticket.id];
        return sum + ((usage?.used || 0) * ticket.price);
      }, 0);
          ...
          historyData.history.forEach((entry: DailyTicketHistory) => {
            ...
            if (entry.type === 'USED') {
              ...
              todayUsage[entry.ticketId].brokenDetails.push({
                quantity: entry.quantity,
                reason: entry.reason,
                code: entry.code || codeFromReason || entry.ticketCode // Essayer différentes sources possibles du code
              });
            }
          });
        }
  ...
  useEffect(() => {
    const revenue = tickets.reduce((sum: number, t: TicketType) => sum + ((t.usedToday || 0) * t.price), 0);
  ...
  const handleMarkAsBroken = async (ticketId: string, reason: string, code: string) => {
    try {
      ...
      // Recharger les données
      await Promise.all([loadTickets(), loadDailyHistory()]);
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Tabs defaultValue="daily" className="space-y-6">
        ...
        <TabsContent value="daily" className="space-y-6">
            ...
            <Card className="border-2 border-blue-200 bg-blue-50">
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-blue-600">
                  {tickets.reduce((sum: number, t: TicketType) => sum + (t.usedToday || 0), 0)}
                </div>
            ...
            <Card className="border-2 border-red-200 bg-red-50">
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-red-600">
                  {tickets.reduce((sum: number, t: TicketType) => sum + (t.brokenToday || 0), 0)}
                </div>
                      ...
                      <Card className="bg-green-50">
                        ...
                        <CardContent>
                          <Button 
                            ...
                            onClick={() => useTicket(ticket.id, index)}
                                ...
                                >
                                  <div>
                                    <div className="font-medium">
                                      {broken.code || broken.ticketCode || "Code non disponible"}
                                    </div>
                                    <div className="text-sm text-muted-foreground">
                                      {broken.reason}
                                    </div>
                                  ...
                                  <Button
                                    ...
                                    onClick={() => {
                                      const newTickets = [...tickets];
                                      const updatedBrokenTickets = [...(ticket.brokenTickets || [])];
                                      updatedBrokenTickets.splice(index, 1);
                                      
                                      newTickets[index] = {
                                        ...ticket,
                                        brokenToday: (ticket.brokenToday || 0) - 1,
                                        brokenTickets: updatedBrokenTickets
                                      };
                                      setTickets(newTickets);
                                    }}
                                  >
          ...
          {(dailyRevenue > 0 || tickets.reduce((sum: number, t: TicketType) => sum + (t.brokenToday || 0), 0) > 0) && (
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="text-sm text-green-700">
                        {tickets.reduce((sum: number, t: TicketType) => sum + (t.usedToday || 0), 0)} tickets vendus
                        {tickets.reduce((sum: number, t: TicketType) => sum + (t.brokenToday || 0), 0) > 0 &&
                          ` • ${tickets.reduce((sum: number, t: TicketType) => sum + (t.brokenToday || 0), 0)} problèmes`
                        }
                      </div>
                    </div>
                  </div>
